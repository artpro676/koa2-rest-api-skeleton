...
const parseInclude = function (ctx, model) {
    ...
    return _.chain(parsedInclude)
        .map(function (name) {
            ...
            _.each(['on', 'separate', 'limit', 'paranoid', 'include'], (attr) => {
                _.has(relation, attr) && (result[attr] = relation[attr]);
            });
...
const parseWhere = function (ctx, model) {
    ...
    return _.chain(parsedWhere)
        .each(function (value, key) {
            ...
            console.log(`${key} `+ _.get(model,`tableAttributes.${key}.type.constructor.key`));
            ...
            switch (true) {
                ...
                case _.isString(value):
                    ...
                    break; // ILIKE '%hat%'
                case _.isArray(value) && (_.get(model,`tableAttributes.${key}.type.constructor.key`) == 'ARRAY'):
                    parsedWhere[key] = {$overlap: value};
                    break; // && [1, 2] (PG array overlap operator)
                case _.isArray(value):
                    parsedWhere[key] = {$in: value};
                    break;
            }
        })
        .value();
...
const parseSearch = function (ctx, model) {
    ...
    try {
        ...
        throw new AppError(500, `Parameter 'search' is not valid json (${model})`, e.Message)
    }
    _.chain(parsedSearch)
        .each((value, key) => {
            if (!_.hasIn(model.attributes, key)) {
                throw new AppError(400, `Parameter "${key}" in "search" is not valid (${model})`);
            }
            ...
            switch (true) {
                ...
                case _.isString(value):
                    ...
                    break;
                case _.isArray(value) && (_.get(model,`tableAttributes.${key}.type.constructor.key`) == 'ARRAY'):
                    searchFor[key] = {$overlap: value};
                    break; // && [1, 2] (PG array overlap operator)
                case _.isArray(value):
                    searchFor[key] = {$in: value};
                    break;
            }
...
const parseAll = function (ctx, model) {
    ...
    const params:any = _.merge(parsePagination(ctx), {
        where: _.merge(parseWhere(ctx, model), parseSearch(ctx, model)),
    ...
    _.each(['where', 'include', 'order'],(key)=>{
        if(!params[key]){
            delete params[key]
        }
    });
